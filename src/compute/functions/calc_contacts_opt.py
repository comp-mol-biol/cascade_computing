...
from src.compute.config import LOCALSCRATCH, OS_GB, BETA_CONST, LAMBDA_CONST
...
def annotate_and_save_contacts(contacts_ddf, A_mol_sel, B_mol_sel, top, traj, a, b, path, name, bool_bonds, bool_debug,n_workers):
    """
    ...
    u = mda.Universe(top, topology_format='pdb')
    ...
    # read contact-data
    meta_dict = {
        'frame': pd.Series([], dtype='int32'),
        'n_cont': pd.Series([], dtype='int32'),
    ...
    #drop atomistic information (comment if atomistic analysis needed)
    df = df.drop(columns=['cont_a', 'cont_b'])
    ...
    if bool_debug:
        ...
        print(f"[{worker_id}] memory writing df parquet: {psutil.Process().memory_info().rss / 1024**2:.2f} MB", flush=True)
...
def empty_contact_df(bool_bonds):
...
@delayed
def process_frame_chunk_for_split(top, traj, group_a_sel, group_b_sel, d_max, radius, k, chunk_size,bool_debug,bool_bonds,bool_q):
    """
    ...
    if not data['frame']:
        return empty_contact_df(bool_bonds)

    return pd.DataFrame(data)   


def contacts_within_cutoff_fast_parallel_split(top, group_a_sel, group_b_sel, traj_min, traj_lim, chunk_path, chunk_size,bool_debug,bool_bonds,bool_q, d_max=1.0, radius=4.5):
    """
    ...
    delayed_chunks = [
        process_frame_chunk_for_split(top, traj, group_a_sel, group_b_sel, d_max, radius, k, chunk_size,bool_debug,bool_bonds,bool_q)
...
@delayed
def calc_interactions(pair, top, traj, name, path, chunk_path, d_max, traj_min, traj_lim, delta_r, chunk_size, bool_debug=False,bool_bonds=False,bool_q=False,bool_sidechains=False, bool_bb=False, n_workers=1):  
    """
    ...
    contacts_ha_ddf = contacts_within_cutoff_fast_parallel_split(top, A_mol_sel_h, B_mol_sel_h, traj_min, traj_lim, chunk_path, chunk_size, bool_debug,bool_bonds,bool_q,d_max=d_max, radius=delta_r)
     
    return annotate_and_save_contacts(
        contacts_ha_ddf, A_mol_sel, B_mol_sel, top, traj, a, b, path, name,bool_bonds, bool_debug,n_workers)


def calc_contact_mat(top, traj, chains, dim, delta_r, path, chunk_size, chunk_path, name='cont_map', d_max=0.5, traj_min=0, traj_lim=5, bool_sidechains=False, bool_bb=False, bool_debug=False,bool_bonds=False,bool_q=False,n_workers=1):
    """
    ...
    """
    delayed_path_tasks = [
        calc_interactions(pair, top, traj, name, path, chunk_path, d_max, traj_min, traj_lim, delta_r, chunk_size, bool_debug=bool_debug,bool_bonds=bool_bonds,bool_q=bool_q,bool_sidechains=bool_sidechains, bool_bb=bool_bb, n_workers=n_workers) 
        for pair in chains
    ]
...
def eval_contact_mat(res_contact_mat, res_contact_mat_lb, eps_ts=10, bool_breaks=True):
    """
...
def pp_pair(pair, ha_result, lb_result, path, name, eps_ts, bool_breaks,bool_debug):
    """
    ...
    if bool_breaks and lb_parquet_path is not None:
        ...
        _, dt_res_contact_mat = eval_contact_mat(res_contact_matti, res_contact_matti_lb, eps_ts=eps_ts, bool_breaks=bool_breaks)
	...
	#possible intermediate results
        ...
        del res_contact_matti_lb, contact_mat_lb
        release_memory()

    else:
        _, dt_res_contact_mat = eval_contact_mat(res_contact_matti, None, eps_ts=eps_ts, bool_breaks=bool_breaks)
    ...
    del contact_mat, res_contact_matti
    release_memory()
    ...
    if bool_debug:
        ...
        print('WORKER_ID', worker_id.name, flush=True)
        print(f"{worker_id.memory_manager.memory_limit / 1024**3:.2f} GB", flush=True)
        print(f"[{worker_id}] memory writing dt parquet: {psutil.Process().memory_info().rss / 1024**2:.2f} MB", flush=True)

    title_dt = f'dt_{name}_{a}_{b}.parquet'
    dt_res_contact_mat.to_parquet(f'{path}/{title_dt}', index=False)

    del dt_res_contact_mat
    release_memory()
    ...
    if bool_debug:
        ...
        print('WORKER_ID', worker_id.name, flush=True)
        print(f"{worker_id.memory_manager.memory_limit / 1024**3:.2f} GB", flush=True)
        print(f"[{worker_id}] memory after dt parquet: {psutil.Process().memory_info().rss / 1024**2:.2f} MB", flush=True)

    return (a, b, "processed")


def pp_contact_mat(chains, eps_ts, path, ha_tasks, lb_tasks,tmp_path, name='cont_map', bool_breaks=True, bool_debug=False):
    """
...
def run(top, traj, bookkeeping_path, dim, combinations_in,traj_min,traj_max,  args): 
    """
    ...
    tmp_path = f"/{LOCALSCRATCH}/{os.environ['SLURM_JOB_ID']}/tmp_parts"
    os.makedirs(tmp_path, exist_ok=True)

    
    if bool_splid:
        chunk_folder_name = 'traj_parts_' + str(c0) + '_' + str(ck)
        chunk_folder = tmp_path + "/" + chunk_folder_name
    else:
        chunk_folder = tmp_path + "/traj_parts"
    ...
    for pair in combinations_in:
        ...
        out_dir_ha = os.path.join(tmp_path, f"cont_map_{a}_{b}")
        os.makedirs(out_dir_ha, exist_ok=True)

        out_dir_lb = os.path.join(tmp_path, f"lb_cont_map_{a}_{b}")
        os.makedirs(out_dir_lb, exist_ok=True)
    print("Directory creation complete.")



    #local scratch for itermediates
    job_id = os.environ.get('SLURM_JOB_ID', 'local_debug')
    local_dir = f"/{LOCALSCRATCH}/{os.environ['SLURM_JOB_ID']}"
    ...
    threads = 1
    mem_per_worker = (TOTAL_RAM - OS_GB) / n_workers
    mem_str = f"{mem_per_worker:.2f}GB"
    print('memory-per-worker', mem_str)


    dask.config.set({
    # Fail faster so we don't sit in whitespace for 10 minutes
    'distributed.comm.timeouts.connect': '60s', 
    'distributed.comm.timeouts.tcp': '60s',
    'distributed.comm.retry.count': 2, # 2 retries - if it fails twice, it's a code error

    # Stop spilling to the slow network disk
    'distributed.worker.memory.target': 0.8,  # Don't worry until 80%
    'distributed.worker.memory.spill': False, 
    'distributed.worker.memory.pause': 0.85,  # Stop work early to save the worker
    })
    ...
    ha_tasks = calc_contact_mat(
        top, traj, combinations_in, dim, delta_ha, tmp_path,
        chunk_size, chunk_folder, d_max=buffer, traj_min=traj_min, traj_lim=traj_max,
        bool_sidechains=b_sidechains, bool_bb=b_bb, bool_debug=bool_debug,bool_bonds=bool_bonds,bool_q=bool_q, n_workers=n_workers
    )
    ...
    if bool_breaks:
        lb_tasks = calc_contact_mat(
            top, traj, combinations_in, dim, delta_l, tmp_path,
            chunk_size, chunk_folder, name='lb_cont_map', d_max=buffer, 
            traj_min=traj_min, traj_lim=traj_max, bool_sidechains=b_sidechains, bool_bb=b_bb,bool_debug=bool_debug,bool_bonds=bool_bonds,bool_q=bool_q, n_workers=n_workers
        )
    else:
        lb_tasks = None

    # Post-processing
    pp_tasks = pp_contact_mat(combinations_in, eps_ts, bookkeeping_path, ha_tasks, lb_tasks,tmp_path ,bool_breaks=bool_breaks, bool_debug=bool_debug)
    ...
    if os.path.exists(local_dir):
        ...
        print(f"Cleaned up local scratch: {local_dir}", flush=True)
...
def main(args):
    ...
    try:
        run(top, traj, bookkeeping_path, u.dimensions, combinations,traj_min,traj_max, args)
    
    finally: 
        job_id = os.environ.get('SLURM_JOB_ID')
        if job_id:
            local_scratch = f"/{LOCALSCRATCH}/{job_id}"
            if os.path.exists(local_scratch):
                shutil.rmtree(local_scratch, ignore_errors=True)
                print(f"Emergency cleanup: Purged {local_scratch}", flush=True)
    return 0
